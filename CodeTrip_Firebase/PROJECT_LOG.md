...
- [2026-09-06](project-log/2026-09-06.md): CodeRabbit 피드백에 따라 댓글 인덱스가 원본 댓글의 작성자·게시글·여행지 참조와 일치하는지 Rules로 검증하고, 목록·마이페이지가 선택된 콘텐츠의 좋아요만 병렬 조회하도록 보완했습니다. 축제 상세 소개는 운영 시간·장소·참여 방법 중심의 문단으로 나누고 본문의 URL을 클릭 가능한 링크로 표시했으며, 정보 카드 공백·URL 뒤 조사 포함·소개 중복 문제를 보정했습니다. 공식 홈페이지와 SNS 외부 링크는 독립된 줄과 아이콘으로 표시하고 한글 도메인도 분리했으며, 운영 시간의 `※` 안내·계절·요일·월별 구간을 줄바꿈했습니다. 여행지·행사 목록에는 5페이지 단위 이동을 추가했고, 여행지 카드는 행사 카드와 같은 `EXPLORE →` 패턴 및 반응형 페이지 수로 통일했습니다. 기능·수정·문제 해결마다 작업 로그를 남기는 규칙도 명확히 했습니다.
