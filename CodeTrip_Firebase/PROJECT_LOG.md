...
- [2026-09-10](project-log/2026-09-10.md): AI 플랜 재생성 시 저장 대상 폴더가 조건 기반 코스를 폴더 기반으로 강제하던 문제를 보완하고, 재생성 조건 변경 뒤에는 새 지역 후보로 코스를 생성하도록 분리했습니다. 혼자 여행 결과에 친구 동행 표현이 섞인 AI 응답은 프롬프트와 서버 정규화로 방지했으며, 연인·가족·친구 여행은 화면·생성 검증·서버 입력에서 최소 2명을 보장하도록 했습니다. 최초 수동 검증의 누락과 재검증 기준은 트러블슈팅 색인에 기록했습니다.
