...
## 18. 2026.07.24 넓은 지역 입력 시 권역 분산 규칙 보강

### 배경

사용자가 `서울`처럼 넓은 지역을 입력했을 때 AI 여행 코스가 성북구 중심으로 반복 생성되는 문제가 있었습니다. 넓은 지역 입력은 특정 구 하나가 아니라 여러 후보 권역을 검토한 뒤, 실제 하루 동선에 맞는 인접 권역을 선택하는 방식이 더 적절합니다.

### 프롬프트 보강 방향

- 넓은 지역명만 입력된 경우 특정 구/동에 고정하지 않도록 명시합니다.
- 관광공사 후보 장소가 제공된 경우 후보를 우선 활용하되, 같은 권역에 과도하게 몰리지 않도록 안내합니다.
- 실제 하루 일정은 이동 피로도를 고려해 인접한 1~3개 권역 안에서 구성하도록 제한합니다.
- 서울의 경우 성북구/성북동 반복 생성을 피하고, 종로구, 중구, 마포구, 용산구, 성동구, 서대문구, 송파구, 강남구, 영등포구 등 다양한 권역 후보를 먼저 검토하도록 명시합니다.

### 기대 효과

- `서울` 입력 시 매번 성북구 중심으로 생성되는 편중 완화
- 관광공사 API 후보 장소 활용도 증가
- 사용자 입력 지역의 범위와 실제 추천 코스의 지역 범위가 더 자연스럽게 일치
