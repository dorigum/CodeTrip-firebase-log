...
- **처리**: 두 경로의 좋아요 사용자 ID를 중복 없이 합산하고, 조회·정렬·토글 후 반환하는 수에도 같은 합산 결과를 사용합니다. 좋아요 요청 중에는 임시 증감 대신 중복 클릭만 막아, 최종 합산 수가 도착할 때까지 화면 수치가 흔들리지 않게 합니다.
- **확인 기준**: 기존 좋아요가 있는 게시글·댓글에서 다른 계정이 좋아요를 취소했다 다시 눌러도 기존 수와 새 수가 함께 유지되어야 합니다.
