...
- **처리**: 댓글 신규 생성과 게시글 좋아요 추가를 Functions RTDB 트리거에서 처리해 작성자 개인 알림을 생성합니다. 댓글 ID 또는 게시글·좋아요 사용자 조합을 알림 ID로 사용해 재시도 중복을 막고, 본인 상호작용은 제외합니다.
- **확인 기준**: 서로 다른 두 계정으로 게시글을 작성한 뒤 다른 계정에서 댓글·좋아요를 남기면 작성자 계정의 Header에 각각 알림이 표시되어야 합니다. 작성자 자신의 댓글·좋아요에는 알림이 생기지 않아야 합니다.
