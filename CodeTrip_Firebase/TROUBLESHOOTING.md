...
- **처리**: 하단 행사·축제소개가 존재할 때 상단의 중복 설명 문단만 숨기고, 홈페이지 본문에서 첫 HTTP(S) URL도 링크 대상으로 사용하도록 보완했습니다. 하단 상세 줄 간격도 상단 정보와 맞췄습니다.
- **확인 기준**: 중복 설명이 한 번만 보이고, 공식 홈페이지 카드가 실제 URL을 새 탭으로 열며, 상세 본문 줄 간격이 일관돼야 합니다.
