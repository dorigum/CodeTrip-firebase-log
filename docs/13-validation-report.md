...
| 2026-08-22 | `feature/trip-date-planning` | AI 여행 계획 날짜 입력과 마이페이지 일정 수정 진입점 | 부분 통과. AI Planner에 여행 시작일·종료일 입력을 추가하고, 날짜와 일수를 동기화하도록 했습니다. 생성 컨텍스트와 Functions 프롬프트에 여행 날짜를 포함하며, 새 AI 코스 폴더 저장 시 폴더 일정으로 반영되도록 했습니다. 마이페이지 `Folder_Notes` 영역에는 `Travel_Schedule` 카드와 `Edit_Date` 진입점을 추가했습니다. | `src/pages/AiPlanner.jsx`, `functions/index.js`, `src/api/wishlistApi.js`, `src/pages/MyPage.jsx`, `docs/10-ai-harness-engineering.md`, `CodeTrip_Firebase/project-log/2026-08-22.md` | `npm run lint`, `npm run build`, 로컬 AI 생성·저장·폴더 날짜 반영·마이페이지 일정 수정 수동 검증이 필요합니다. |
