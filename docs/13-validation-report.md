...
| 2026-08-16 | `security/gemini-functions-proxy` | Gemini Callable Function 프록시 배포 검증 | 부분 통과. `GEMINI_API_KEY` Functions Secret 등록, `generateTripPlan` v2 callable Function 배포, Artifact Registry 1일 cleanup policy 설정, Hosting 배포, 배포 산출물 Gemini 키 원문 미포함, 미인증 Callable 요청 401 차단 확인 | Firebase CLI 배포 로그, `functions:list` 결과, `dist secret scan: not found`, `callable unauth blocked: 401`, Hosting URL `https://dorigum-codetrip.web.app` | 로그인 상태의 실제 AI 생성 smoke test와 기존 노출 가능 Gemini 키 폐기·거부 확인은 별도 수행 필요 |
...
| 2026-08-16 | `security/gemini-functions-proxy` | Gemini API 키 교체 및 Secret version 2 smoke test | 부분 통과. 기존 2026-07-22 키를 `Gemini API Key_temp`로 식별 가능하게 이름 변경하고, 2026-08-16 신규 키를 `.env`와 Firebase Secret `GEMINI_API_KEY` version 2로 등록했다. `generateTripPlan` 재배포 후 신규 키 최소 호출 200과 Callable 인증 호출 성공을 확인했다. | AI Studio 키 목록의 마스킹된 신규 키 suffix `...7nFQ`, Secret version 2 생성 로그, `Gemini new key minimal call status: 200`, `Callable new secret smoke test: success` | 기존 `Gemini API Key_temp` 삭제 후 기존 키 거부 확인 필요 |
