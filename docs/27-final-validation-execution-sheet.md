...
| AI-SEC-05 | 키 교체·만료 완료 | 기존 클라이언트 노출 가능 키는 실제로 폐기 또는 만료 처리되어야 한다. 서버는 신규 Secret을 사용해야 하며, 기존 키가 거부되는지 마스킹된 식별자로 확인한다. | 부분 통과 | 신규 키 suffix `...7nFQ`를 Secret version 2로 등록하고 Function 재배포 후 smoke test 성공 | 기존 `Gemini API Key_temp` 삭제 및 거부 확인 필요 |
