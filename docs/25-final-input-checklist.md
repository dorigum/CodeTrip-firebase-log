...
이 문서는 CodeTrip 1차 심사 자료를 한국관광 콘텐츠랩에 제출하기 직전, 제출 페이지와 기능설명서에 실제로 입력해야 하는 값의 준비 상태를 점검하기 위한 체크표다.
...
| 우선순위 | 리스크 | 원인 | 예방 방법 |
|---|---|---|---|
| P0 | 로그인 불가 | 테스트 계정 미생성, 비밀번호 오류, 보호 기능 접근 실패 | 제출 전 별도 계정으로 로그인·AI·마이페이지·커뮤니티 접근을 확인한다. |
| P0 | OpenAPI 정보 누락 | 인코딩키·디코딩키 미확인, 활용 API 목록 불일치 | 제출 페이지 입력 전 공공데이터포털 키와 코드 endpoint 목록을 대조한다. |
| P0 | 기능설명서 양식 위반 | 제공 양식 수정, 5페이지 초과, 12pt 미만, 10MB 초과, PDF 열람 불가 | 최종 PDF 변환 후 페이지 수, 글자 크기, 용량, 열람을 다시 확인한다. |
| P0 | 팀명 placeholder 잔존 | 기능설명서 1페이지의 `[접수 팀명 입력]` 미교체 | 접수 페이지 팀명을 확인한 뒤 PPTX와 PDF에 반영한다. |
| P1 | 최신 배포 미반영 | 로컬 최신 작업이 Firebase Hosting에 반영되지 않음 | 제출 직전 서비스 URL smoke test와 배포 시점을 기록한다. |
| P1 | 화면 캡처 설득력 부족 | 비로그인 화면만 사용해 AI·마이페이지·커뮤니티 완성도 증빙이 약함 | 테스트 계정 준비 후 로그인 내부 화면 캡처로 교체한다. |
| P1 | 지역 특화 입력 불일치 | 제출 페이지와 기능설명서의 지역 특화 여부가 다름 | `docs/23-regional-specialization-strategy.md` 기준으로 최종 선택을 먼저 확정한다. |

## 최종 확인 기록 양식

최종 제출 직전 아래 형식으로 확인 결과를 남긴다. 실제 비밀번호와 인증키는 기록하지 않는다.

| 확인일 | 확인자 | 확인 범위 | 결과 | 후속 조치 |
|---|---|---|---|---|
| YYYY-MM-DD | 이름 | FI-01~FI-16 | 미진행 | 최종 제출 전 작성 |

## 현재 다음 조치

1. 접수 페이지에서 팀명과 최종 팀원 정보를 확인한다.
2. 테스트 전용 계정을 생성하고 `docs/19-test-account-verification.md` 기준으로 검증한다.
3. OpenAPI 인코딩키·디코딩키와 활용 API 목록을 제출 페이지 기준으로 확인한다.
4. 지역 특화 여부를 전국 단위 제출로 확정할지, 특정 지역 특화로 전환할지 결정한다.
5. 로그인 후 내부 화면 캡처를 확보해 기능설명서 3페이지 이미지를 교체한다.
6. 최종 PPTX/PDF를 생성하고 `docs/17-submission-artifact-manifest.md`에 checksum과 전달 위치를 기록한다.
