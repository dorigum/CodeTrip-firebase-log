...
| FI-07 | 지역 특화 여부 | 확인됨: 전국 단위 제출, 지역 특화 서비스 `아니오` | 제출 직전에도 같은 선택이 제출 페이지와 기능설명서에 동일하게 반영되어 있는지 확인한다. | 제출 페이지, 기능설명서 | 신청 정보 화면, `docs/23-regional-specialization-strategy.md`, 최종 PDF |
...
| FI-17 | 동일 서비스 중복 출품 여부 | 미확인 | 동일 서비스로 타 부문 또는 타 공사 주관 지원 사업 수상·수혜 이력이 없는지 확인한다. | 제출 페이지, 팀 내부 확인 | 제출 직전 확인 기록 |
| FI-18 | 제출 부문·양식 일치 여부 | 미확인 | 웹·앱 개발 부문 제출 페이지와 웹·앱 개발 부문 기능설명서 양식이 일치한다. | 제출 페이지, 최종 PDF | 제출 직전 확인 기록 |
| FI-19 | 마감 전 수정 가능 시간 확보 | 미확인 | 2026-09-21 16:00 전 제출·수정 가능 시간을 확보하고, 마감 직전 변경을 피한다. | 제출 일정 | 제출 직전 확인 기록 |
...
| P1 | 부문·양식 오첨부 | 웹·앱 개발 부문이 아닌 다른 부문 양식 또는 파일을 첨부함 | 제출 직전 부문명, 파일명, PDF 내용, 양식 종류를 다시 확인한다. |
| P1 | 마감 직전 수정 실패 | 마감 직전 이메일 인증·파일 업로드·입력값 수정이 지연됨 | 제출 마감 전 여유 시간을 확보하고 최종 수정 가능 시간을 기록한다. |
