...
이 문서는 CodeTrip 기능설명서, 시연, 제출 페이지를 기준으로 심사자가 물어볼 수 있는 질문과 답변 방향을 정리합니다. 답변은 현재 구현·문서·검증 상태를 기준으로 하며, 측정되지 않은 성과나 아직 완료되지 않은 기능을 완료된 것처럼 표현하지 않습니다.

## 답변 원칙

- 먼저 사용자 문제와 서비스 흐름을 설명합니다.
- 기술명은 선택 이유와 사용자 가치로 연결해 설명합니다.
- 현재 MVP 범위와 향후 발전계획을 분리합니다.
- 보안·성능·검증의 미완료 항목은 숨기지 않고 백로그와 보완 계획으로 설명합니다.
- 인증키, 비밀번호, 개인 이메일, 팀원 개인정보는 답변이나 문서에 노출하지 않습니다.
...
| 다른 여행 서비스와 차별점은 무엇인가요? | 공공 관광 데이터 기반 탐색, 개인 저장, Gemini AI 일정화, 커뮤니티 공유를 한 흐름으로 묶은 점입니다. 단순 정보 조회가 아니라 탐색 결과가 개인 여행 계획과 사용자 참여로 이어집니다. | `docs/24-submission-copywriting.md` |

## 서비스 완성도 관련 질문

| 질문 | 답변 방향 | 연결 문서 |
|---|---|---|
| 현재 어디까지 구현되어 있나요? | 여행지 탐색, 축제 조회, 상세 정보, 인증, 찜·폴더, AI 일정 생성, 커뮤니티 흐름을 MVP 범위로 정리했습니다. 다만 최종 제출 전 테스트 계정 검증과 로그인 후 내부 화면 캡처가 필요합니다. | `docs/18-submission-gap-analysis.md`, `docs/28-submission-readiness-dashboard.md` |
| 심사자가 직접 테스트할 수 있나요? | Firebase Hosting 공개 URL을 제출하고, 로그인 기능이 필요한 경우 지정 형식의 테스트 전용 계정을 제출 페이지에 입력합니다. 최종 제출 전 URL smoke test와 테스트 계정 검증을 수행해야 합니다. | `docs/19-test-account-verification.md`, `docs/21-service-url-verification.md` |
| 실패 화면이나 빈 결과도 고려했나요? | 여행지와 축제 조회는 로딩, 빈 결과, 오류 상태를 구분해 다루는 방향으로 정리했습니다. 최종 시연 전에는 URL smoke test와 핵심 사용자 흐름 검증으로 확인합니다. | `docs/06-quality-plan.md`, `docs/27-final-validation-execution-sheet.md` |
| 기능설명서의 화면 캡처는 충분한가요? | 현재 초안은 비로그인 공개 화면 중심입니다. 최종 제출 전 테스트 계정이 준비되면 AI 일정, 마이페이지 또는 찜·폴더, 커뮤니티 내부 화면으로 일부 교체하는 것이 필요합니다. | `docs/16-contest-submission-checklist.md`, `docs/26-pptx-final-editing-guide.md` |
| 최종 제출 가능 상태인가요? | 현재는 제출 초안 준비 완료, 최종 제출은 보류 상태입니다. 접수 팀명, 테스트 계정, OpenAPI 인증키, 로그인 후 캡처, 최종 PDF 검증이 끝나야 제출 가능으로 판정합니다. | `docs/28-submission-readiness-dashboard.md`, `docs/27-final-validation-execution-sheet.md` |

## 데이터 활용 관련 질문

| 질문 | 답변 방향 | 연결 문서 |
|---|---|---|
| 한국관광공사 OpenAPI를 어디에 사용했나요? | `KorService2`를 통해 여행지 목록, 키워드 검색, 상세 정보, 상세 이미지, 지역 코드, 축제·행사 정보를 활용하고, `PhotoGalleryService1`의 `galleryList1`을 홈 화면 관광 이미지에 활용합니다. | `docs/20-openapi-submission-verification.md`, `docs/16-contest-submission-checklist.md` |
| OpenAPI 활용이 서비스 핵심과 어떻게 연결되나요? | TourAPI는 여행지 탐색과 상세 정보의 중심 데이터입니다. AI 일정 생성에서도 관광공사 기반 장소와 AI 추천 장소를 구분해 결과 신뢰성을 관리하는 근거로 활용합니다. | `docs/10-ai-harness-engineering.md`, `docs/20-openapi-submission-verification.md` |
| OpenAPI 인증키는 문서에 포함되나요? | 아닙니다. 인코딩키와 디코딩키는 제출 페이지에만 입력하고 저장소에는 기록하지 않습니다. 문서에는 확인 절차와 증빙 위치만 남깁니다. | `docs/20-openapi-submission-verification.md`, `docs/25-final-input-checklist.md` |
| 기타 API는 무엇을 사용하나요? | Open-Meteo는 날씨, Nominatim은 위치명 보조, Kakao Maps는 지도, Gemini API는 일정 초안 생성, Firebase는 인증·DB·배포에 사용합니다. | `docs/04-architecture.md`, `docs/16-contest-submission-checklist.md` |
...
- 기능설명서 문구는 `docs/24-submission-copywriting.md`와 일치해야 합니다.
- 제출 가능 여부는 `docs/27-final-validation-execution-sheet.md`의 차단 조건으로 판단합니다.
- 현재 준비도는 `docs/28-submission-readiness-dashboard.md`에서 확인합니다.
- 최종 답변에서 실제 인증키, 비밀번호, 개인 이메일, 팀원 개인정보를 말하거나 문서에 남기지 않습니다.
