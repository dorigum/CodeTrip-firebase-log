...
Google OAuth는 별도 세션 체계를 만들지 않고 현재 이메일·비밀번호 로그인과 같은 세션 계약을 사용한다.

| 항목 | 기준 |
|---|---|
| 로그인 시작 | OAuth 팝업 또는 리다이렉트 실행 전에 `useAuthStore.prepareLogin()`을 호출한다. |
| Firebase Auth persistence | `browserSessionPersistence`를 사용해 브라우저 세션 기준으로 Firebase Auth 상태를 유지한다. |
| 앱 로그인 완료 | Firebase 인증 성공 후 기존 `login()` 경로로 앱 사용자 상태를 반영한다. |
| 세션 만료 | 기존 2시간 만료 정책을 동일하게 적용한다. |
| `trip_token` 정리 | 로그아웃 또는 만료 시 기존 정리 로직을 그대로 적용한다. |

현재 `trip_token`은 `localStorage`에 저장되므로 탭 종료만으로 자동 삭제된다고 가정하지 않는다. 탭 종료를 세션 종료로 정의하려면 `trip_token` 저장소와 GO-V04·GO-V06 검증 기준을 함께 변경해야 한다.

## 프로필 upsert 기준

Google OAuth 로그인 후 `users/{uid}` 프로필이 없을 때만 기본 프로필을 생성한다. 이미 프로필이 있으면 로그인 정보는 병합하되 사용자가 관리하는 필드는 덮어쓰지 않는다.
...
기존 프로필이 있는 경우 `name`, `profileImg`, `favoriteRegions`, `created_at`처럼 사용자가 직접 관리하거나 최초 생성 의미가 있는 필드는 보존한다. 반복 로그인 시에는 `updated_at`, 마지막 로그인 provider 같은 시스템 관리 필드만 갱신한다.

이메일은 화면 표시 이름 fallback으로 사용하지 않는다. Google 계정에 `displayName`이 없는 경우에도 기능설명서 캡처와 렌더링 화면에 이메일이 이름처럼 노출되지 않아야 한다.

## 동일 이메일 계정 연결 정책

동일 이메일로 이메일·비밀번호 계정과 Google 계정이 충돌하는 경우 자동 병합하지 않는다. 기본 정책은 `차단 후 복구 안내`다.

| 상황 | 처리 |
|---|---|
| 기존 이메일·비밀번호 계정이 있고 Google 로그인을 시도함 | 기존 방식으로 먼저 로그인하라고 안내하고, 이후 명시적 계정 연결 플로우에서 `linkWithCredential`을 검토한다. |
| 기존 Google 계정이 있고 이메일·비밀번호 가입을 시도함 | Google 로그인으로 계속하라고 안내하거나, 별도 비밀번호 설정 복구 플로우를 제공한다. |
| 복수 Provider 연결을 지원하기로 결정한 경우 | 기존 Provider로 재인증한 뒤 `linkWithCredential`을 사용하고, 실패 시 계정 충돌 복구 안내를 제공한다. |
| 충돌 계정 테스트 | 동일 이메일을 가진 이메일·비밀번호 계정과 Google 계정 테스트 케이스를 별도 준비한다. |

복구 안내는 지원하는 로그인 방식별로 분리한다. 이메일·비밀번호 사용자는 비밀번호 재설정 또는 기존 로그인 후 연결을 안내하고, Google 사용자는 Google 로그인 유지 또는 계정 연결 대기 상태를 안내한다.

## Settings 화면 예외 처리

OAuth 계정은 이메일·비밀번호로 재인증할 수 없으므로 비밀번호 변경 기능에서 예외 처리가 필요하다. `authApi.updatePassword`를 수정할 때는 `authUser.providerData`에 `password` provider가 연결되어 있는지 먼저 확인하고, 최근 로그인 요구 오류를 명확히 처리한다.
...
Settings UI와 `authApi.updatePassword`는 같은 정책을 사용해야 한다. UI에서 숨긴 기능이 API에서 실행되거나, API가 거부하는 기능이 UI에 표시되지 않도록 검증한다.
...
| GO-V03 | 프로필 생성 | 신규 프로필은 기본값으로 생성되고 기존 프로필은 사용자 관리 필드를 보존한 채 병합된다. |
| GO-V04 | 세션 만료 | `browserSessionPersistence`와 기존 2시간 세션 정책이 함께 적용된다. |
| GO-V05 | 보호 라우트 접근 | AI Planner, 마이페이지, 커뮤니티 접근이 가능하다. |
| GO-V06 | 로그아웃 | Firebase Auth와 로컬 세션, `trip_token`이 함께 정리된다. |
| GO-V07 | 비밀번호 변경 예외 | Google-only 계정에서는 비밀번호 변경 UI가 잘못 표시되지 않고, 복수 Provider 계정은 `password` provider 기준으로 처리된다. |
| GO-V08 | 캡처 보안 | Google 계정 이메일이 이름 fallback으로 렌더링되거나 기능설명서 캡처에 노출되지 않는다. |
| GO-V09 | 반복 로그인 | 동일 Google 계정으로 반복 로그인해도 `created_at`, `favoriteRegions`, 사용자 지정 `name`, `profileImg`가 덮어써지지 않는다. |
| GO-V10 | displayName 없음 | `authUser.displayName`이 없는 Google 계정도 중립 placeholder로 표시되고 이메일은 노출되지 않는다. |
| GO-V11 | 동일 이메일 충돌 | 이메일·비밀번호 계정과 Google 계정의 동일 이메일 충돌 시 자동 병합하지 않고 복구 안내를 제공한다. |
