...
- 신규 Google 계정 회원가입·홈 이동, 기존 Google 계정 로그인, 기존 계정 재가입 안내, 로그인·로그아웃 흐름을 확인했습니다.
- Google OAuth 계정의 게시글·게시글 댓글·여행지 댓글 좋아요가 추가·취소·누적되는 것을 확인했습니다.
- Google-only 계정에는 비밀번호 변경 폼 대신 Google 계정 관리 안내가 표시되는 것을 확인했습니다.
- 인증 복원 깜빡임 보정과 Google-only 비밀번호 변경 차단은 현재 브랜치를 배포한 뒤 운영 URL에서 한 번 더 확인해야 합니다.

## 현재 상태

Google OAuth는 `코드 구현 완료·핵심 수동 점검 완료` 상태입니다. 공모전 제출 전 필수 차단 항목은 아니며, 이메일·비밀번호 테스트 계정과 기존 세션 흐름을 유지한 선택 로그인으로 추가했습니다. 이후 배포마다 인증 복원, Google 로그인·로그아웃, 좋아요 권한, Google-only 비밀번호 안내를 운영 URL에서 재확인합니다.
