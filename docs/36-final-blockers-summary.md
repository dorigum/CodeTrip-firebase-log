...
문서 체계와 제출용 초안은 준비되어 있지만, 제출 가능 판정을 내리려면 외부 제출 페이지와 실제 서비스 상태에서 확인해야 하는 값이 남아 있다. 특히 접수 팀명, 테스트 계정, OpenAPI 인증키, 로그인 후 화면 캡처, 최종 PDF 검증은 문서만으로 완료 처리할 수 없다. Gemini 호출부는 Firebase Callable Function 프록시로 전환되어 배포되었고, Secret 등록, 미인증 차단, 인증 성공 smoke test도 확인했다. 남은 AI 보안 차단 항목은 기존 노출 가능 키 폐기·거부 확인이다.
...
| G6 AI 공개 시연 보안 | 공개 배포 환경에서 클라이언트 키 노출 차단 필요 | Callable Function·Secret·Hosting 배포 완료, 배포 산출물 키 미포함, 미인증 401 차단, 인증 성공 smoke test 확인, 기존 키 폐기 검증 필요 | 부분 완료 |
