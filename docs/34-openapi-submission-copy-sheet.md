...
| ID | 대조 항목 | 확인 기준 | 결과 |
|---|---|---|---|
| API-01 | 제출 계정 키 확인 | 공공데이터포털에서 제출에 사용할 인증키를 확인했습니다. 인코딩키·디코딩키가 별도 필드로 표시되지 않으면 제출 페이지 안내에 따라 일반 인증키 또는 URL 인코딩 값을 사용합니다. 원문은 저장소에 기록하지 않습니다. | 부분 확인 |
| API-02 | 국문 관광정보 서비스 신청 확인 | 제출 계정에서 `KorService2` 활용 가능한 키인지 확인합니다. | 부분 확인 |
| API-03 | 관광사진 정보 신청 확인 | 제출 계정에서 `PhotoGalleryService1` 활용 가능한 키인지 확인합니다. | 부분 확인 |
| API-04 | 배포 환경변수 대조 | 배포 서비스가 사용하는 `VITE_TRAVEL_INFO_API_KEY`, `VITE_GALLERY_API_KEY`가 제출 계정 기준과 일치하는지 확인합니다. | 부분 확인 |
...
| API-07 | 공개 화면 동작 확인 | 홈, 탐색, 축제, 상세 화면에서 관광 데이터가 표시됩니다. | 부분 확인 |
| API-08 | 인증키 노출 점검 | 기능설명서, 캡처, PR, 문서에 인증키 원문이 없습니다. | 문서 기준 통과, 최종 PDF 재확인 필요 |
...
현재 코드 근거 기준으로 `KorService2`와 `PhotoGalleryService1` 활용 목록은 정리되어 있습니다. 사용자는 2026-08-16에 국문 관광정보 서비스와 관광사진 정보 키를 확인하고 로컬·배포 서비스에서 관광 데이터 출력이 정상임을 확인했습니다. 다만 제출 페이지에 입력할 인증키 원문, 제출 페이지의 인코딩키·디코딩키 요구 방식, 최종 PDF 5페이지의 API 목록 대조는 제출 직전 사용자가 직접 확인해야 합니다.
