...
이 문서는 CodeTrip MVP에서 확인되었거나 문서상 관리 대상으로 정의된 기술 부채를 추적한다. 기술 부채는 결함과 동일하지 않으며, 현재 MVP 목표를 달성하는 데 허용했지만 이후 품질·보안·운영 리스크를 줄이기 위해 해결해야 하는 항목이다.
...
- 기술 부채가 해결되었다고 주장하려면 코드 변경, 검증 결과, 문서 갱신 중 최소 하나 이상의 증빙 위치를 함께 남긴다.
- 측정값이 없는 항목은 `resolved`로 표시하지 않는다.
- MVP 범위에서 해결하지 않기로 결정한 항목은 `accepted`로 바꾸고, 수용 이유와 재검토 조건을 `decision-log/`에 기록한다.
- 새 기술 부채가 발견되면 이 문서에 먼저 등록하고, WBS 또는 백로그 문서에 실행 작업으로 분리한다.
