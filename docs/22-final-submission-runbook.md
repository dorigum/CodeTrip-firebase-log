...
이 문서는 CodeTrip의 2026 관광데이터 활용 공모전 웹·앱 개발 부문 1차 심사 자료를 최종 제출하기 직전에 따라야 할 실행 순서를 정리합니다. 개별 검증 문서가 많아진 상태에서 제출 누락을 줄이기 위한 최상위 실행 문서입니다.

## 사용 시점

아래 조건 중 하나라도 해당하면 이 문서를 기준으로 제출 준비 상태를 재점검합니다.
...
아래 값은 저장소에 실제 원문을 남기지 않습니다. 제출 페이지나 최종 PDF에 반영할 때만 사용합니다.
...
아래 항목이 모두 `통과` 또는 `완료` 상태가 되어야 제출 가능으로 판정합니다.
...
제출을 완료하면 아래 내용을 저장소 문서에 남깁니다. 단, 인증키, 비밀번호, 개인 이메일, 개인정보는 기록하지 않습니다.
...
현재 문서와 산출물 기준으로 남은 핵심 미완료 항목은 다음과 같습니다.
