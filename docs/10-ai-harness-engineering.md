...
CodeTrip의 Gemini 여행 일정 생성을 단순 API 호출이 아니라, 입력 검증·컨텍스트 구성·모델 호출·출력 검증·저장·평가가 연결된 신뢰 가능한 실행 계층으로 운영한다.

하네스의 성공 기준은 “Gemini가 응답했다”가 아니라 다음을 모두 만족하는 것이다.

- 인증된 사용자만 요청할 수 있다.
- 모델에 전달되는 입력이 유효하고 제한되어 있다.
- TourAPI 검증 장소와 AI 추천 장소가 구분된다.
- 응답이 CodeTrip의 실제 도메인 스키마를 만족한다.
- 오류·재시도·재생성·fallback이 구분된다.
- 모델·프롬프트·검증 결과를 추적할 수 있다.
...
향후 `src/ai/` 또는 Functions 내부에 다음 책임을 분리한다.
...
- 시작 시간은 종료 시간보다 이르다.
- 배열·문자열 길이에 상한을 둔다.
- `preferredPlaces`는 필요한 개수만 제한적으로 전달한다.
- HTML, 스크립트, 불필요한 개인정보는 제거한다.

### 출력 계약

현재 CodeTrip 화면과 저장 로직의 기준은 `days[].items`다. 모든 신규 서버 구현도 이 계약을 유지해야 하며, `days[].schedule`을 사용하려면 별도 변환 계층을 둔다.
...
JSON 여부, 필수 필드, 타입, 배열 구조, 시간 형식을 확인한다.
...
`contentId`, `contentid`, `content_id`를 내부 기준인 `contentId`로 통일한다. 출처는 다음 세 종류로 통일한다.
...
재생성은 최대 1회로 제한한다. 재생성 프롬프트에는 검증 실패 항목만 전달하고, 사용자의 원래 조건은 유지한다.
...
- 클라이언트에서 Callable Function을 호출하는 방향은 적절하다.
- Functions에서 Secret Manager를 사용하려는 방향은 API 키 보호에 적합하다.
- 인증, 입력 검증, timeout, retry, 사용자 메시지 변환을 고려했다.
- Gemini 응답을 서버에서 파싱하려는 계층 분리가 시작되어 있다.

### 병합 전 수정 필수

1. **응답 스키마 충돌**: 제안 Function은 `days[].schedule`을 생성하지만 현재 CodeTrip은 `days[].items`를 사용한다. 화면과 저장 로직을 깨뜨릴 수 있으므로 기존 `items` 계약을 유지한다.
...
- 브라우저 번들에 Gemini API 키가 포함되지 않는다.
- 프론트엔드와 Functions의 출력 스키마가 일치한다.
- 정상·오류·빈 입력·timeout·429·잘못된 JSON을 검증한다.
- 공식 장소와 AI 추천 장소가 UI와 저장 데이터에서 구분된다.
- prompt version, model, validation status, retry count를 추적할 수 있다.
- 고정 평가 케이스의 결과와 실패 사유가 문서화된다.
