...
- 여행 시작일과 종료일은 둘 다 비어 있거나 둘 다 제공되어야 합니다. 한쪽만 제공된 요청은 서버와 저장 단계에서 거부합니다.
- 폴더 기반 생성에서는 선택한 폴더의 `start_date`, `end_date`, `durationDays`를 동일한 유효 범위로 정규화합니다. 시작일만 있는 폴더는 1일 코스로 처리하고, 5일을 초과하는 폴더 일정은 AI 생성 가능 범위에 맞춰 최대 5일 범위로 조정합니다.
- 인원 수 1~10명
- 시작 시간은 종료 시간보다 이릅니다.
- 배열·문자열 길이에 상한을 둡니다.
