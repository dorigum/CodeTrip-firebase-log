...
이 문서는 CodeTrip의 2026 관광데이터 활용 공모전 웹·앱 개발 부문 1차 심사 자료 제출 준비 상태를 한눈에 보기 위한 요약 문서입니다. 세부 절차는 각 기준 문서에 따르며, 이 문서는 최종 의사결정과 남은 차단 항목 파악을 돕기 위한 현황판입니다.
...
아래 수치는 완료 주장이 아니라 현재 작업 상태를 빠르게 보기 위한 참고값입니다.

| 구분 | 준비도 | 해석 |
|---|---:|---|
| 문서 체계 | 90% | 문서 구조와 연결은 갖춰졌고 링크 무결성 검사도 통과했습니다. |
| 기능설명서 초안 | 70% | 5페이지 초안은 있으나 팀명, 로그인 후 캡처, 최종 입력값 반영이 남아 있습니다. |
| 제출 페이지 입력 준비 | 45% | 입력 항목은 정리됐지만 실제 팀명, 계정, OpenAPI 키 확인이 필요합니다. |
| 서비스 검증 | 40% | 검증 절차는 있으나 최종 URL smoke test와 테스트 계정 검증은 미실행입니다. |
| 최종 제출 가능성 | 55% | 방향과 문서는 준비됐지만 차단 조건이 아직 남아 있습니다. |

## 제출 직전 차단 항목

아래 항목은 완료 전까지 최종 제출을 보류합니다.
...
| B-09 | 제출 직전 차단 조건 통과 | 미실행 | `docs/27-final-validation-execution-sheet.md`의 필수 차단 ID가 모두 `통과` 상태여야 합니다. `부분 통과`, `실패`, `미실행`은 제출 보류입니다. 필수 범위는 `VE-01`, `VE-06`~`VE-12`, `AI-SEC-01`~`AI-SEC-11`, `PDF-01`~`PDF-06`, `PDF-09`, `SUB-01`~`SUB-03`입니다. | `docs/27-final-validation-execution-sheet.md`, `docs/32-service-url-smoke-test-runbook.md`, `docs/36-final-blockers-summary.md` |
...
|---|---:|---|---|
| 서비스 기획력 | 30 | 사용자 문제, 서비스 개요, 차별성 문구가 정리되어 있습니다. | 지역 특화 여부 최종 결정, PPTX 최종 문구 반영 |
| 서비스 완성도 | 30 | 배포 URL, 공개 화면 캡처, 핵심 기능 흐름 초안이 있습니다. Gemini 호출부는 Callable Function 프록시로 전환되어 배포되었고 인증 성공 smoke test를 통과했습니다. | 테스트 계정 전체 UI 검증, 로그인 후 내부 화면 캡처, 최종 smoke test |
| 데이터 활용 적절성 | 20 | TourAPI 활용 endpoint와 기타 API 목록이 정리되어 있습니다. | OpenAPI 인증키와 제출 계정 기준 활용 API 최종 대조 |
| 서비스 발전성 | 20 | Functions 프록시, 권한 검증, 성능 측정, E2E 테스트, 지역 큐레이션 발전계획이 정리되어 있습니다. | 최종 PPTX에서 기술 부채를 과장 없이 발전계획으로 표현 |

## 다음 실행 순서

1. 접수 페이지에서 팀명과 최종 팀원 정보를 확인합니다.
2. `docs/30-user-provided-submission-inputs.md` 기준으로 사용자 제공 입력값과 민감정보 기록 금지 범위를 확인합니다.
3. 지역 특화 여부가 제출 페이지와 기능설명서에서 `아니오`로 일치하는지 최종 확인합니다.
4. 테스트 전용 계정을 생성하고 로그인·AI·마이페이지·커뮤니티 접근을 검증합니다.
5. OpenAPI 인증키와 활용 API 목록을 제출 계정 기준으로 확인하고, 배포 서비스 키와 제출 계정 키의 마스킹된 식별자를 대조합니다.
6. 공개 URL에서 Gemini 생성 기능을 제공하려면 `docs/27-final-validation-execution-sheet.md`의 `AI-SEC-01`~`AI-SEC-11`을 모두 실행합니다.
   - Functions 프록시 코드 반영 여부와 배포 여부를 확인합니다.
   - 서버 측 Secret 사용 여부를 확인합니다.
   - 클라이언트 번들 내 Gemini 키 직접 참조를 검사합니다.
   - Firebase Auth ID token을 서버에서 검증하고, 미인증 요청이 Gemini 호출 전에 거부되는지 확인합니다.
   - 검증된 `uid` 기준 사용자별 rate limit, quota, 동시 요청 제한을 확인합니다.
   - 서버 측 입력 크기·형식·민감정보 필터를 확인합니다.
   - 요청·응답·Functions 로그의 원문 비기록과 민감정보 마스킹을 확인합니다.
   - 기존 노출 가능 키가 실제로 폐기 또는 만료되었고, 서버가 신규 Secret을 사용하는지 확인합니다.
   - 위 조건 중 하나라도 미완료이면 공개 URL의 Gemini 신규 생성을 제출 통과 상태로 보지 않습니다.
7. 로그인 후 내부 화면 캡처를 확보합니다.
8. `docs/26-pptx-final-editing-guide.md` 기준으로 최종 PPTX를 수정합니다.
9. 최종 PDF로 변환하고 `docs/27-final-validation-execution-sheet.md` 기준으로 검증합니다.
10. 최종 산출물 checksum과 전달 위치를 `docs/17-submission-artifact-manifest.md`에 기록합니다.
11. 중복 출품, 부문 오첨부, 마감 후 수정 불가 리스크를 제출 직전에 다시 확인합니다.

## 상태 갱신 규칙

- 이 문서는 실제 확인이 끝날 때마다 갱신합니다.
- 민감정보 원문은 기록하지 않습니다.
- 준비도 수치는 참고값이며, 최종 제출 가능 여부는 `docs/27-final-validation-execution-sheet.md`의 차단 조건 통과 여부로 판단합니다. 특히 `AI-SEC` 항목은 공개 URL에서 Gemini 생성 기능을 제공할지 여부를 결정하는 제출 차단 조건으로 봅니다.
- 최종 제출을 완료하면 제출 완료일, 최종 PDF 파일명, checksum, 제출 직전 검증 결과를 갱신합니다.
