...
가장 제출본에 가까운 산출물은 `output/contest/CodeTrip_2026_관광데이터_공모전_기능설명서_5페이지_제출항목정리_초안.pdf`다. 이 파일은 PDF 5페이지, 394335 bytes로 확인되었고, 전체 1~5번 슬라이드 PNG 렌더링을 통해 원본 가이드 문구가 제거된 읽기 가능한 초안임을 확인했다.

다만 접수 팀명, 테스트 계정, OpenAPI 인증키, 로그인 후 내부 기능 화면, 최종 12포인트 이상 육안 검증은 아직 미완료다.
...
| PDF 용량 | 10MB 미만 | 394335 bytes 확인 | 충족 | `docs/17-submission-artifact-manifest.md` | 최종본 변환 후 재확인 |
| 글자 크기 | 12포인트 이상 | 작성 시 12포인트 이상을 목표로 구성했으나 전체 텍스트 객체 자동 검증은 미완료 | 부분 충족 | `output/contest/rendered/submission-draft-all/` | PowerPoint 육안 검증 또는 텍스트 객체 검사 |
