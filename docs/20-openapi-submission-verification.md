...
아래 내용은 인증키 원문을 기록하지 않고, 실제 확인된 상태만 남깁니다.

| 항목 | 확인 상태 | 비고 |
|---|---|---|
| 국문 관광정보 서비스 | 부분 확인 | 사용자가 공공데이터포털에서 `KorService2` 인증키를 확인하고 로컬 `.env`에 반영한 뒤 서비스 데이터가 정상 출력됨을 확인했습니다. |
| 관광사진 정보 | 부분 확인 | 사용자가 관광사진 API 키가 현재 사용 중인 키와 동일함을 확인했습니다. |
| 배포 서비스 동작 | 부분 확인 | 2026-08-16 배포 후 홈, 탐색, 상세, 축제 화면에서 관광 데이터가 표시되는 것을 확인했습니다. |
| 인증키 원문 기록 | 통과 | 인증키 원문은 저장소 문서와 커밋에 기록하지 않습니다. |
| 제출 페이지 입력값 | 미확인 | 공공데이터포털 또는 제출 페이지에서 요구하는 인코딩키·디코딩키 입력은 제출 페이지에서만 직접 수행해야 합니다. |
| 최종 PDF API 목록 | 미확인 | 기능설명서 최종 PDF 5페이지에 `KorService2`, `PhotoGalleryService1`가 반영된 뒤 대조합니다. |

현재 상태는 `부분 통과`입니다. 코드 근거와 배포 화면 동작은 확인되었지만, 제출 페이지의 인증키 입력과 최종 PDF 문구 대조는 사용자가 제출 직전에 확인해야 합니다.

## 검증 절차

1. 공공데이터포털에서 공모전 제출에 사용할 한국관광공사 OpenAPI 신청자 계정으로 로그인합니다.
2. 사용 중인 국문 관광정보 서비스와 관광사진 정보 서비스의 인증키를 확인합니다. 포털 화면이 인코딩키·디코딩키를 별도 필드로 제공하지 않고 일반 인증키만 제공하는 경우, 제출 페이지 안내에 따라 해당 키를 직접 입력하거나 URL 인코딩한 값을 별도 계산해 입력합니다.
...
- 키 일치 여부는 `확인함`, `불일치`, `미확인`처럼 상태만 기록하고, 필요하면 마스킹된 끝자리도 저장소에는 남기지 않습니다.
