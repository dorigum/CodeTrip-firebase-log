...
| BL-14 | 기능설명서 PPTX 작성 및 PDF 변환 | P0 | ready | 제출 안내, 기능설명서 양식 | 제공 양식 기반 기능설명서 작성, PDF 변환 후 열람 확인, 제출 산출물 manifest 작성 | 비바이너리 manifest: 파일명, 생성일, 양식 버전, checksum, 외부 전달 위치 |

## 추천 처리 순서

1. BL-02: Firebase Rules 권한 시나리오 검증표 작성
2. BL-04: 번들 크기 기준선 측정표 작성
3. BL-03: 라우트 단위 코드 스플리팅 적용 여부 결정 및 구현
4. BL-05: 핵심 사용자 여정 수동 검증 보고서 작성
5. BL-09: 주요 의사결정 로그 작성
6. BL-01: Gemini API Functions 프록시 이전 설계와 구현
