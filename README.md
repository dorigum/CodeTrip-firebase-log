...
React / Vite
  ...
  -> Firebase Callable Function: generateTripPlan
       ...
       -> Gemini API
  -> Firebase Scheduled Function: syncTourApiUpdates
       -> Functions Secret: TOUR_API_SERVICE_KEY
       -> TourAPI 최신 여행지 데이터
...
- 사용자 선호 지역 기반 빠른 필터
- 찜 버튼을 통한 위시리스트 저장
- 비회원 위시리스트 클릭 시 로그인 안내 모달 제공
- 로그인 후 기존 탐색 페이지·검색어·필터·페이지 맥락 복귀
...
- 축제 상세 조회와 위시리스트 저장
- Home 화면에서는 현재 날짜 기준으로 가까운 축제 정보를 우선 표시
...
TourAPI 신규 데이터 감지 결과는 공용 피드로 관리합니다.
...
- 프로필 이미지는 `profileImages/{uid}/profile` 고정 경로에 덮어쓰기 방식으로 저장합니다.
- 게시글 첨부 이미지는 게시글별 경로에 저장합니다.
- Realtime Database와 Firebase Auth에는 Storage 다운로드 URL만 저장합니다.
- data URL을 직접 저장하지 않아 프로필 URL 길이 제한과 DB 비대화 문제를 줄입니다.
...
Gemini API 키와 TourAPI 신규 데이터 감지용 키는 루트 `.env`에 두지 않고 Firebase Functions Secret에 등록합니다.
